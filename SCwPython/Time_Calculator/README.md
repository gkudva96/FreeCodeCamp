...
If the function is given the optional starting day of the week parameter, then the output includes the day of the week of the result.
...
The unit tests for this project are in `test_module.py`. The tests are imported from `test_module.py` to `main.py` for your convenience. The tests will run automatically when the `main.py` script is run.